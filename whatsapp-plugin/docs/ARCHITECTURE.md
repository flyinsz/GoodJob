...
它已经具备独立插件控制台、Baileys 免费通道、Meta Cloud API 官方通道、联系人与会话、实时消息、AI 翻译、CRM Sandbox、MySQL 生产配置约束和基础运维能力，但尚不满足公网多用户、多租户或多实例生产发布门禁。

“候选版”不代表已经通过真实生产环境验收。当前 MySQL 驱动、旧 PostgreSQL 数据迁移器和 Meta 接口已实现，自动化测试也已覆盖关键契约，但目标 MySQL、真实 Meta 凭据、两个真实 Baileys 账号的端到端验收仍待执行。
...
- 本机开发可使用 PGlite，生产运行时强制与 CRM 共用 MySQL。
...
| Express API | 参数校验、GoodJob CRM 签名会话身份、领域操作、Meta Webhook、生产静态资源托管 | 当前为个人账号所有权模型，不是完整组织级 RBAC 或多租户边界 |
...
| Repository | MySQL 参数化 SQL、账号隔离、幂等和审计 | Communication 表与 CRM 表共库但边界独立 |
| GoodJob CRM Adapter | 读取客户与待办，写入客户互动、待办和站内信；本地 `crm_contacts` 仅作为独立运行回退 | CRM 写失败进入自动化失败记录并释放幂等声明供重试 |
...
2. 生产环境校验 MySQL、32 字节 Base64 主密钥、Demo 禁用和官方 Meta Graph 地址。
...
- API Key 使用主密钥加密后存入 MySQL，不返回明文到浏览器。
...
- 生产环境强制 `DATABASE_CLIENT=mysql`，并与 CRM 复用有效的 `DATABASE_URL`。
- 环境变量严格解析，非法布尔值、端口、Origin、URL 和主密钥启动即失败。
- 生产强制关闭 Demo 数据与 Demo Provider。
- 生产禁止把 Meta Graph 地址改到非官方端点。
- 使用 `communication_schema_migrations` 的版本化迁移；MySQL DDL 采用可重入列/索引检查。
- MySQL 事务使用固定 Pool Connection，失败回滚后释放连接。
...
| 门禁 | 当前风险 | 完成标准 |
| --- | --- | --- |
| 组织级 RBAC | 当前已验证 GoodJob CRM 会话并按个人账号隔离，但没有组织角色矩阵 | 管理员、运营、销售、只读角色及账号级授权策略 |
| `tenant_id` 多租户模型 | 数据库没有租户级强制隔离 | 所有领域表、查询、唯一约束和任务携带租户身份 |
| Socket ACL 与账号房间 | 所有连接接收全局事件 | 鉴权握手、租户/账号房间和服务端授权检查 |
| 出站 Outbox | Meta 入站已持久化并支持失败恢复；出站消息在不确定状态仍需人工对账 | 事务 Outbox、幂等发送、超时对账和死信处理 |
...
| CRM 冲突队列 | 已接入 GoodJob CRM 客户、互动、待办和站内信，失败可重试；缺少独立冲突处理队列 | 字段冲突、身份合并和人工处理界面 |
| 真实渠道与数据库 E2E | Mock/本机通过不能代表目标环境 | 目标 MySQL、真实 Meta 测试号、两个隔离 Baileys 账号完整验收 |
