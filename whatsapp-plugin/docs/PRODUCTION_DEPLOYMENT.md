...
当前没有应用层登录鉴权、RBAC、租户隔离和 Socket ACL，因此不能把整个服务直接暴露到公网。需要 Meta 官方 Webhook 时，只允许通过反向代理选择性公开 `/api/webhooks/meta/*`，其他页面、API 和 Socket.IO 必须继续受 VPN、内网或外部身份访问网关保护。
...
- PostgreSQL 必须是受备份、访问控制和监控保护的独立服务。
...
- 目标 PostgreSQL 版本已经用同一发布包运行迁移和集成验收。
...
自动化测试通过不等于真实渠道通过。首次上线前还必须分别完成目标 PostgreSQL、真实 Meta 测试号码和两个隔离 Baileys 账号的端到端验收。
...
| `DATABASE_CLIENT` | 必须为 `postgres` | 生产会拒绝 PGlite |
| `DATABASE_URL` | 必需 | 通过 Secret 注入；建议启用 PostgreSQL TLS 和最小权限 |
...
AI Provider 和 Meta App/账号凭据通过控制台/API 写入 PostgreSQL，并由 `SESSION_MASTER_KEY` 加密，不通过相应业务环境变量注入。
...
- 数据库只允许应用网段和迁移任务访问。
- 使用专用数据库和专用角色，不复用超级用户。
- 开启服务端 TLS，并在 `DATABASE_URL` 中按组织策略校验证书。
- 为连接数、存储、WAL、慢查询和备份失败配置告警。
...
迁移器先创建 `schema_migrations`，再按版本顺序执行未应用迁移。每个迁移在独立事务中运行；失败时该迁移回滚，后续版本不会执行。

注意事项：

- 当前没有跨进程 advisory lock，只允许一个迁移任务运行。
- 不要在多个应用副本启动时开启 `AUTO_MIGRATE`。
- 迁移成功后查询 `schema_migrations`，确认版本和名称与发布包一致。
...
2. 确认 `schema_migrations` 已是当前版本。若需要升级，先把迁移作为独立变更执行并验收，再重新开始本清理流程。
...
| 端点 | 成功含义 | 不包含的保证 |
| --- | --- | --- |
| `/api/health/live` | Node.js 进程能够响应 | 不检查 PostgreSQL、WhatsApp、Meta 或 AI |
| `/api/health/ready` | 未进入关闭流程且 PostgreSQL `SELECT 1` 成功 | 不保证任何渠道账号已连接 |
...
当前没有 Prometheus 指标端点、持久任务队列面板或多实例聚合。上线前应由现有监控平台基于健康端点、受保护 API、PostgreSQL 指标和日志建立临时监控；完整指标体系仍属于生产完善项。

## 10. 备份与恢复

### 10.1 必须备份的内容

- PostgreSQL 全库：账号、加密 AuthState、联系人、会话、消息、译文、路由、加密 AI/Meta 凭据和审计。
...
### 10.2 PostgreSQL 备份示例

优先使用 `PGSERVICE`、`.pgpass` 或平台身份认证，避免在命令行显示连接密码：

```bash
umask 077
pg_dump \
  --dbname="service=whatsapp_crm" \
  --format=custom \
  --no-owner \
  --no-acl \
  --file="whatsapp-crm-backup.dump"
sha256sum "whatsapp-crm-backup.dump"
pg_restore --list "whatsapp-crm-backup.dump" >/dev/null
```
...
2. 创建空的恢复数据库，不直接覆盖当前库做第一次验证。
3. 使用单事务恢复：

```bash
pg_restore \
  --dbname="service=whatsapp_crm_restore" \
  --single-transaction \
  --exit-on-error \
  --no-owner \
  --no-acl \
  "whatsapp-crm-backup.dump"
```

4. 使用备份对应的 `SESSION_MASTER_KEY` 启动隔离实例。
5. 检查 `schema_migrations`、readiness、账号/联系人/会话/消息计数和加密凭据可解密性，不输出具体业务内容。
...
1. `/api/health/live` 和 `/api/health/ready` 返回成功，数据库类型为 PostgreSQL，Demo Provider 为关闭。
2. `schema_migrations` 与发布包一致，日志没有启动错误或持续重试风暴。
...
- 真实 CRM Adapter；
- 目标 PostgreSQL、真实 Meta 和两个隔离 Baileys 账号的完整端到端验收。
